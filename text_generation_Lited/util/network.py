...
class ENCODER(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.condition:
        #     # c with the size (16x31), expand to the same size as x
        #     c = c.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, x.shape[2], x.shape[3])
        #     # concatenate input image x and condition c
...
class NetG(nn.Module):
    ...
    def forward(self, z, c=None):
        if self.condition:
            ...
            # concatenate input embedding z and condition c
...
class Discriminator(nn.Module):
    ...
    def __init__(self, ndf, nDep, ncIn=3, bSigm=True, condition=True):
        ...
        self.adv = nn.Sequential(
            nn.Conv2d(of, 1, 5, 2, 2),
            nn.Sigmoid()
            )

        self.material = nn.Sequential(
            nn.Conv2d(of, opt.z_material, 5, 2, 2),
            )

        self.color = nn.Sequential(
            nn.Conv2d(of, opt.z_color, 5, 2, 2),
            )
